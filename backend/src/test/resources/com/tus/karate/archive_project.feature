...
Scenario: Admin archives project after issue closed
  * def projectName = 'Archive Project ' + java.util.UUID.randomUUID()
  ...
  * def devToken = response.token
  ...
  {
    "name": "#(projectName)",
    "description": "Archive test project"
  }
  """
  When method post
  Then status 201
  * def projectId = response.id
  ...
  """
  {
    ...
    "reporterId": 2,
    "title": "Archive Issue",
    "description": "Issue before archive",
    "priority": "LOW"
  }
  """
  When method post
  ...
  * def issueId = response.id
  ...
  Given url baseUrl + '/api/projects/' + projectId + '/archive'
  And header Authorization = 'Bearer ' + adminToken
  When method put
  Then status 200